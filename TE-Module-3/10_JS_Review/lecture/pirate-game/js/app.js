...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('body').addEventListener('keyup', (event) => {
        //console log to find event key name
    ...
    resetBtnRef.addEventListener('click', (event) => {
        resetGame();
        // const framed = document.getElementById('frame')
        // framed.classList.add('boat')
    })
...
function moveShipDown() {
    const ship = getShipLocation();
    const shipIndex = Array.from(ship.parentNode.children).indexOf(ship);
    const lowerNeighbor = ship.parentNode.nextElementSibling
    if (lowerNeighbor != null) {
        const newSpot = lowerNeighbor.children[shipIndex];
        if (newSpot != null && !newSpot.classList.contains('pirate')) {
            ship.classList.remove('boat')
            newSpot.classList.add('boat')
        }
    }
...
function moveShipUp() {
    const ship = getShipLocation();
    const shipIndex = Array.from(ship.parentNode.children).indexOf(ship);
    const upperNeighbor = ship.parentNode.previousElementSibling
    if (upperNeighbor != null) {
        const newSpot = upperNeighbor.children[shipIndex];
        if (newSpot != null && !newSpot.classList.contains('pirate')) {
            ship.classList.remove('boat')
            newSpot.classList.add('boat')
        }
...
function buildSquare(row) {
    const squareRef = document.createElement('div');
    squareRef.setAttribute('class', 'square')
    row.appendChild(squareRef)
}

function createObstacles() {
    const rows = document.querySelectorAll('.row')
    console.log(rows);
    //got to convert from the node list aboe (queryselectorAll) to array

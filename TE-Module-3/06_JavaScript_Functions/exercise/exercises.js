 ...
 * @returns {[]} newArray with Happy prepended
 */
function makeHappy(array){
    return array.map((current)=>{return 'Happy ' + current})
}
/**
 ...
 * @param {string[]} arrayAddress object with full info
 * @param {string} streetNumber number of address
 * @param {string} streetName name of street
 * @param {string} streetType Rd. Dr. etc
 * @param {string} city city name
 * @param {string} state state name
 * @param {string} zip zipcode
 * @returns {string[]}  mailing address of strings 
 */
function getFullAddressesOfProperties(arrayAddress){
    return arrayAddress.map((item)=>{
        return [item.streetNumber, item.streetName, item.streetType, item.city, item.state, item.zip].join(" ");
})
}
/** 

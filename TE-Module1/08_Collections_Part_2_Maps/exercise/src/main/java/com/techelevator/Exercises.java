...
//make the map then if string passed in aka animal name is a key in the map
		//use .equalsIgnoreCase()
		// if found return the value. else return "unknown."
		...
		for (Map.Entry<String, String> currentEntry : animals.entrySet()) {
			if (currentEntry.getKey().equalsIgnoreCase(animalName)) {
	...
	public double isItOnSale(String itemNumber) {
		...
		discountPercent.put("KITCHEN6073", .40);
		discountPercent.put("BEDROOM3434", .60);
		discountPercent.put("BATH0073", 0.15);

		for (Map.Entry<String, Double> currentEntry : discountPercent.entrySet()) {
			if (currentEntry.getKey().equalsIgnoreCase(itemNumber)) {
				return currentEntry.getValue();
			}
			if (itemNumber == null) {
				return 0.00;
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		if (petersMoney > 0 && paulsMoney < 1000){
			int amountToRemoveFromPeter = petersMoney / 2;
			peterPaul.put("Peter", petersMoney - amountToRemoveFromPeter);// name the key and change the value
			peterPaul.put("Paul", paulsMoney + amountToRemoveFromPeter);
		}
		return peterPaul;
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		if (petersMoney >= 5000 && paulsMoney >= 10000){
			int newPeter = petersMoney - (int)(petersMoney * .25);
			int newPaul = paulsMoney - (int)(paulsMoney * .25);
			int comboMoney = (int)(petersMoney * .25) + (int)(paulsMoney * .25);;
			peterPaul.put("Peter", newPeter);
			peterPaul.put("Paul", newPaul);
			peterPaul.put("PeterPaulPartnership", (comboMoney));
		return peterPaul;
		}
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		// make map, loop through array, if map contains value "letter" then do alt version with substring last letter
		Map<String, String> wordAndLetter = new HashMap<>();
		String wordFirstLetter = "";
		String wordLastletter = "";
		for (int i = 0; i < words.length; i++) {
			wordFirstLetter = words[i].substring(0, 1);
			wordLastletter = words[i].substring(words[i].length() - 1);
				wordAndLetter.put(wordFirstLetter, wordLastletter);
			}
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		Map<String, Boolean> wordCounter = new HashMap<>();
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
		...
		// make new map and see if map1 contains sku in map 2 then add together
		Map<String, Integer> consolidate = new HashMap<>();
		for (Map.Entry<String, Integer> currentEntryOne : mainWarehouse.entrySet()) {
			consolidate.put(currentEntryOne.getKey(), currentEntryOne.getValue());

		}
		for (Map.Entry<String, Integer> currentEntry : remoteWarehouse.entrySet()) {

			if (mainWarehouse.containsKey(currentEntry.getKey())) {
					int newValue = mainWarehouse.get(currentEntry.getKey());
					//int mainVal = mainWarehouse.get(currentEntry);
					consolidate.put(currentEntry.getKey(), currentEntry.getValue() + newValue);
				} else {
					consolidate.put(currentEntry.getKey(), currentEntry.getValue());
				}
			}
		return consolidate;
	}

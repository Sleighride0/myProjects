...
public class Exercise01_StoreOrders {
    ...
    public int getCheesePizzaRevenue(int[] orders) {
        ...
        int smallPizzaPrice = 8;
        int medPizzaPrice = 11;
        int larPizzaPrice = 14;
        int pizzaRev = 0;
        for (int i = 0; i < orders.length; i++) {
            if (orders[i] == 10){
                smallPizzaCount++;
        ...
        pizzaRev = (smallPizzaCount * smallPizzaPrice) + (medPizzaCount * medPizzaPrice) +
                (larPizzaPrice * larPizzaCount);
        return pizzaRev;
    }
}

...
public class Exercise03_Shirts {
    ...
    public char[] buildBulkOrder(int numberOfShirts) {
        char[] bulkOrderSpread = new char[numberOfShirts];
        for (int i = 0; i < numberOfShirts; i++) {
            if (bulkOrderSpread.length < 1) {
                return bulkOrderSpread;
            }
            if (i == 0) {
                bulkOrderSpread[0] = 'S';
            }
            if (i > 0) {
                if (bulkOrderSpread[i - 1] == 'S') {
                    bulkOrderSpread[i] = 'M';
                }
                if (bulkOrderSpread[i - 1] == 'M') {
                    bulkOrderSpread[i] = 'L';
                }
                if (bulkOrderSpread[i - 1] == 'L') {
                    bulkOrderSpread[i] = 'S';
                }
            }
        }
